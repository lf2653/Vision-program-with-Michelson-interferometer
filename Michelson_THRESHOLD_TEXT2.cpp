...
int main(int argc, char** argv)
{
	...
	while (1)
	{
		...
		if (num >-2)
		{
			...
			char string1[10];//设置字符串缓存位置
			char string2[10];//设置字符串缓存位置
			char string3[10];//设置字符串缓存位置
			p = 1059 - (num / 0.0937);
			n = 1 + p*39.4692*0.0937;
			string numString("Num of list:");
			string pString("P:");
			string nString("N:");
			sprintf_s(string1, "%d", num);      
			sprintf_s(string2, "%1.f", p);      
			sprintf_s(string3, "%6.f", n);      
			numString += string1;                    // 在后加入数值字符串
			pString += string2;                    // 在后加入数值字符串
			nString += string3;                    // 在后加入数值字符串
			// 将帧率信息写在输出帧上
